...
<body>
	...
	<script>
		const priceData = <c:forEach items="${productCalData}" var="product">[${product.sum1}, ${product.sum2}, 
			${product.sum3}, ${product.sum4}, ${product.sum5}, ${product.sum6}]</c:forEach>;
		const touchData = <c:forEach items="${touchData}" var="product">[${product.sum1}, ${product.sum2}, 
			${product.sum3}]</c:forEach>;
	  	...
	  	var ctx2 = document.getElementById("myChart4");
   		...
   		var moneyChart = {
			type: 'bar',
			labels: ["의류&잡화", "뷰티", "생필품", "가전/디지털", "애완용품", "티켓"],
	    	datasets: [{
			   label: '중고거래 매출(원)',
			   data: priceData,
      	...
      	var touchChart = {
   	    	...
   	    	datasets: [{
   			   label: '중고거래 수익(원)',
   			   data: touchData,
   			   backgroundColor: 'rgba(90, 189, 198, 0.8)',
   			}]
   	  	};
