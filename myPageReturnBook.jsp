...
<body>
	...
	<c:forEach items="${myRentalBookingData}" var="mypagebookVO">
		<input type="hidden" name="num" value="${mypagebookVO.book_num}">
		<div id="carouselExampleIndicators" class="carousel slide"
			data-ride="carousel">
			<div class="carousel-inner">
				<div class="carousel-item active">
					<img src="img/${mypagebookVO.book_img}" class="d-block w-100"
		...
		<div class="conten">
			<h6>
				제품 상태 <strong> <c:choose>
						<c:when test="${mypagebookVO.book_condition == 0}">
							반납
						</c:when>

						<c:when test="${mypagebookVO.book_condition == 1}">

							<c:forEach items="${myRentalBookingData}" var="mypagebookVO">
								<jsp:useBean id="noww" class="java.util.Date" />
								<fmt:formatDate value="${noww}" pattern="yyyy-MM-dd" var="today" />
								<c:choose>
									<c:when test="${today >= mypagebookVO.return_date}">
									연체중
								</c:when>
									<c:otherwise>
									대여중
						...
						<c:when test="${mypagebookVO.book_condition == 2}">
							예약중
						</c:when>
					</c:choose>
				</strong>
			...
			<h4>
				<strong><c:out value="${mypagebookVO.book_name}" /></strong>
			</h4>
			<span> <c:choose>
					<c:when test='${mypagebookVO.category == "1"}'>
						전공책
						</c:when>
					<c:when test='${mypagebookVO.category == "2"}'>
						자격증책
						</c:when>
				</c:choose>
			</span>
	...
	<c:forEach items="${myRentalBookingData}" var="mypagebookVO">
		...
		<input type="hidden" name="num"
			value="${mypagebookVO.reservation_num}">
		...
		<c:choose>
			...
			<c:when test="${mypagebookVO.book_condition == 1}">
				...
				<c:choose>
					<c:when test="${today >= mypagebookVO.return_date}">
						<div class="conten">
					<h5>
						<strong>나의 대여 /예약 실황</strong>
					</h5>
					</div>
					<div id="demo"></div>
						<button class="no">연체중 어서 반납하세요</button>

						<input type="hidden" class="rental"
							value="<fmt:formatDate value="${mypagebookVO.rental_date}" pattern="yyyy-M-d" />" />
						<input type="hidden" class="return"
							value="<fmt:formatDate value="${mypagebookVO.return_date}" pattern="yyyy-M-d" />" />
					</c:when>
			...
			<c:when test="${mypagebookVO.book_condition == 2}">
				...
				<form method="post"
					action="Controller?cmd=bookingCancleAction&number=${mypagebookVO.reservation_num}&number2=${mypagebookVO.book_num}"
					class="rental">
					<a
						href="Controller?cmd=bookingCancleAction&number=${mypagebookVO.reservation_num}"
						onclick="return confirm('정말로 예약을 취소하시겠습니까?')">
